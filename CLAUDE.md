...
web/
  ...
  static/            nocturne.css (vendored design system), app.css, app.js
...
- **Design system:** the look comes from Nocturne, vendored to `web/static/nocturne.css` from the Claude Design project "PitStop SSO UI mockups" — re-export rather than hand-editing it. `web/static/app.css` holds only what the design system doesn't cover: the three OKLCH expiry-status steps and the app's own layouts.
